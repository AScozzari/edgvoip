...
export class ExtensionService {
  ...
  async getExtensionStatus(extensionId: string, tenantId?: string): Promise<{
    extension: string;
    registered: boolean;
    ip?: string;
    port?: number;
    ...
    try {
      ...
      const result = await client.query(
        `SELECT e.extension, e.display_name, t.sip_domain 
         FROM extensions e 
      ...
      try {
        const { FreeSWITCHService } = await import('./freeswitch.service');
        ...
        const registration = registrations.find((reg: any) => 
          reg.user === ext.extension && reg.realm === ext.sip_domain
        );
