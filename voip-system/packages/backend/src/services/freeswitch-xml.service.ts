...
<document type="freeswitch/xml">
  ...
  async processXmlRequest(params: FreeSwitchXmlParams): Promise<string> {
    try {
      ...
      const tagAttrName = anyParams.tag_attr_name as string | undefined;
      const tagAttrVal = anyParams.tag_attr_val as string | undefined;
      ...
      if (!sipDomain && section === 'dialplan' && context) {
        ...
        try {
          const tenantBySlug = await client.query(
            'SELECT id, slug, name, sip_domain FROM tenants WHERE slug = $1 AND status = $2',
            [context, 'active']
          );
