...
class ConferenceService {
    ...
    async createConferenceRoom(conferenceRoomData) {
        return (0, database_1.withTransaction)(async (client) => {
            // Check if extension is already taken
            const existingRoom = await client.query(`SELECT id FROM conference_rooms WHERE extension = $1 AND tenant_id = $2`, [conferenceRoomData.extension, conferenceRoomData.tenant_id]);
            if (existingRoom.rows.length > 0) {
                throw new Error(`Extension ${conferenceRoomData.extension} is already in use`);
            }
            const result = await client.query(`INSERT INTO conference_rooms (tenant_id, name, description, extension, pin, moderator_pin, max_members, record, record_path, moh_sound, announce_sound, settings, enabled)
    ...
    async getConferenceRoomById(id, tenantId) {
        const client = await (0, database_1.getClient)();
        try {
            const result = await client.query(`SELECT * FROM conference_rooms WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
    ...
    async getConferenceRoomByExtension(extension, tenantId) {
        const client = await (0, database_1.getClient)();
        try {
            const result = await client.query(`SELECT * FROM conference_rooms WHERE extension = $1 AND tenant_id = $2 AND enabled = true`, [extension, tenantId]);
            return result.rows.length > 0 ? this.mapRowToConferenceRoom(result.rows[0]) : undefined;
        }
        finally {
            client.release();
    ...
    async listConferenceRooms(tenantId) {
        const client = await (0, database_1.getClient)();
        try {
            const result = await client.query(`SELECT * FROM conference_rooms WHERE tenant_id = $1 ORDER BY name ASC`, [tenantId]);
            return result.rows.map((row) => this.mapRowToConferenceRoom(row));
        }
        finally {
            client.release();
        }
    }
    async updateConferenceRoom(id, tenantId, updateData) {
        return (0, database_1.withTransaction)(async (client) => {
            // Check if extension is already taken by another room
            if (updateData.extension) {
                const existingRoom = await client.query(`SELECT id FROM conference_rooms WHERE extension = $1 AND tenant_id = $2 AND id != $3`, [updateData.extension, tenantId, id]);
                if (existingRoom.rows.length > 0) {
    ...
    async deleteConferenceRoom(id, tenantId) {
        return (0, database_1.withTransaction)(async (client) => {
            const result = await client.query(`DELETE FROM conference_rooms WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
            return result.rowCount > 0;
        });
    }
    async getConferenceMembers(conferenceId, tenantId) {
        const client = await (0, database_1.getClient)();
        try {
            const result = await client.query(`SELECT cm.* FROM conference_members cm
         JOIN conference_rooms cr ON cm.conference_id = cr.id
         WHERE cm.conference_id = $1 AND cr.tenant_id = $2 AND cm.leave_time IS NULL
         ORDER BY cm.join_time ASC`, [conferenceId, tenantId]);
            return result.rows.map((row) => this.mapRowToConferenceMember(row));
        }
        finally {
            client.release();
        }
    }
    async addConferenceMember(conferenceId, tenantId, memberData) {
        return (0, database_1.withTransaction)(async (client) => {
            // Verify conference exists and belongs to tenant
            const conference = await client.query(`SELECT id FROM conference_rooms WHERE id = $1 AND tenant_id = $2`, [conferenceId, tenantId]);
            if (conference.rows.length === 0) {
                throw new Error('Conference room not found');
            }
            const result = await client.query(`INSERT INTO conference_members (conference_id, extension, caller_id_name, caller_id_number, is_moderator)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`, [
                conferenceId,
    ...
    async removeConferenceMember(conferenceId, tenantId, memberId) {
        return (0, database_1.withTransaction)(async (client) => {
            const result = await client.query(`UPDATE conference_members 
    ...
    async updateMemberStatus(conferenceId, tenantId, memberId, updates) {
        return (0, database_1.withTransaction)(async (client) => {
            const updateFields = [];
            const updateValues = [];
            let paramCount = 1;
    ...
    async validateConferenceRoom(conferenceRoom) {
        const errors = [];
    ...
    async getActiveConferenceRooms(tenantId) {
        const client = await (0, database_1.getClient)();
        try {
            const result = await client.query(`SELECT * FROM conference_rooms WHERE tenant_id = $1 AND enabled = true ORDER BY extension ASC`, [tenantId]);
            return result.rows.map((row) => this.mapRowToConferenceRoom(row));
        }
        finally {
            client.release();
    ...
    async generateFreeSwitchXml(conferenceRoom) {
...
<include>
  <extension name="conference_${conferenceRoom.extension}">
    ...
    async getConferenceStatistics(conferenceId, tenantId) {
        const client = await (0, database_1.getClient)();
        try {
         ...
         JOIN conference_rooms cr ON cm.conference_id = cr.id
         WHERE cm.conference_id = $1 AND cr.tenant_id = $2`, [conferenceId, tenantId]);
