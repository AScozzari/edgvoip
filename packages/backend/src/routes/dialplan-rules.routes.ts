import express from 'express';
import { DialplanRulesService } from '../services/dialplan-rules.service';
import { authenticateToken, AuthRequest } from '../middleware/auth';
import { requireTenantOwnerOrMaster } from '../middleware/tenant-auth';
import { asyncHandler, successResponse, errorResponse } from '../utils/response';

const router = express.Router();
const dialplanRulesService = new DialplanRulesService();

// Apply authentication to all routes
router.use(authenticateToken);

/**
 * GET /api/dialplan/rules
 * List all dialplan rules for a tenant or specific context
 */
router.get(
  '/rules',
  requireTenantOwnerOrMaster,
  asyncHandler(async (req: AuthRequest, res) => {
    const { tenant_id, context } = req.query;

    if (!tenant_id) {
      return errorResponse(res, 'tenant_id is required', 400);
    }

    let rules;
    if (context) {
      rules = await dialplanRulesService.getRulesByContext(tenant_id as string, context as string);
    } else {
      rules = await dialplanRulesService.getRulesByTenant(tenant_id as string);
    }

    successResponse(res, { rules, total: rules.length }, 'Dialplan rules retrieved successfully');
  })
);

/**
 * POST /api/dialplan/rules
 * Create a new dialplan rule
 */
router.post(
  '/rules',
  requireTenantOwnerOrMaster,
  asyncHandler(async (req: AuthRequest, res) => {
    const ruleData = req.body;

    // Validate pattern
    if (!dialplanRulesService.validatePattern(ruleData.match_pattern)) {
      return errorResponse(res, 'Invalid regex pattern', 400, 'INVALID_PATTERN');
    }

    const rule = await dialplanRulesService.createRule(ruleData);

    successResponse(res, rule, 'Dialplan rule created successfully', 201);
  })
);

/**
 * PUT /api/dialplan/rules/:id
 * Update an existing dialplan rule
 */
router.put(
  '/rules/:id',
  requireTenantOwnerOrMaster,
  asyncHandler(async (req: AuthRequest, res) => {
    const { id } = req.params;
    const updates = req.body;

    // Validate pattern if provided
    if (updates.match_pattern && !dialplanRulesService.validatePattern(updates.match_pattern)) {
      return errorResponse(res, 'Invalid regex pattern', 400, 'INVALID_PATTERN');
    }

    const rule = await dialplanRulesService.updateRule(id, updates);

    successResponse(res, rule, 'Dialplan rule updated successfully');
  })
);

/**
 * DELETE /api/dialplan/rules/:id
 * Delete a dialplan rule
 */
router.delete(
  '/rules/:id',
  requireTenantOwnerOrMaster,
  asyncHandler(async (req: AuthRequest, res) => {
    const { id } = req.params;

    await dialplanRulesService.deleteRule(id);

    successResponse(res, { deleted: true }, 'Dialplan rule deleted successfully');
  })
);

/**
 * POST /api/dialplan/rules/:id/test
 * Test a dialplan rule against a phone number
 */
router.post(
  '/rules/:id/test',
  requireTenantOwnerOrMaster,
  asyncHandler(async (req: AuthRequest, res) => {
    const { id } = req.params;
    const { number } = req.body;

    if (!number) {
      return errorResponse(res, 'number is required', 400);
    }

    // Get the rule
    const rules = await dialplanRulesService.getRulesByTenant(req.user!.tenant_id);
    const rule = rules.find((r) => r.id === id);

    if (!rule) {
      return errorResponse(res, 'Rule not found', 404);
    }

    const matches = dialplanRulesService.testRule(rule, number);

    successResponse(
      res,
      {
        matches,
        rule_name: rule.name,
        pattern: rule.match_pattern,
        number_tested: number,
      },
      matches ? 'Number matches rule' : 'Number does not match rule'
    );
  })
);

/**
 * POST /api/dialplan/test-pattern
 * Test a regex pattern against a phone number (without creating a rule)
 */
router.post(
  '/test-pattern',
  authenticateToken,
  asyncHandler(async (req: AuthRequest, res) => {
    const { pattern, number } = req.body;

    if (!pattern || !number) {
      return errorResponse(res, 'pattern and number are required', 400);
    }

    // Validate pattern
    if (!dialplanRulesService.validatePattern(pattern)) {
      return errorResponse(res, 'Invalid regex pattern', 400, 'INVALID_PATTERN');
    }

    const result = dialplanRulesService.testPattern(pattern, number);

    successResponse(
      res,
      {
        ...result,
        pattern,
        number,
      },
      result.match ? 'Pattern matches' : 'Pattern does not match'
    );
  })
);

export default router;

