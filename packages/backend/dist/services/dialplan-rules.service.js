...
exports.DialplanRulesService = void 0;
const database_1 = require("@w3-voip/database");
const uuid_1 = require("uuid");
class DialplanRulesService {
    /**
     * Create default dialplan rules for a specific context
     */
    async createDefaultRulesForContext(tenantId, context) {
    ...
    async createRule(rule) {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO dialplan_rules (
     ...
     */
    async getRulesByContext(tenantId, context) {
        const client = await (0, database_1.getClient)();
        try {
            const result = await client.query(`SELECT * FROM dialplan_rules 
         WHERE tenant_id = $1 AND context = $2 AND enabled = true 
         ORDER BY priority ASC`, [tenantId, context]);
            return result.rows;
        }
        finally {
            await client.release();
    ...
    async getRulesByTenant(tenantId) {
        ...
        try {
            const result = await client.query(`SELECT * FROM dialplan_rules 
         WHERE tenant_id = $1 
         ORDER BY context, priority ASC`, [tenantId]);
            return result.rows;
        }
        finally {
            await client.release();
    ...
    async updateRule(ruleId, updates) {
        ...
        try {
            ...
            values.push(ruleId);
            const result = await client.query(`UPDATE dialplan_rules SET ${fields.join(', ')}, updated_at = NOW() 
...
exports.DialplanRulesService = DialplanRulesService;
//# sourceMappingURL=dialplan-rules.service.js.map