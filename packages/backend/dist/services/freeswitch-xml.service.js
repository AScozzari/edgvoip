...
<document type="freeswitch/xml">
  <section name="directory">
     ...
     */
    async generateDialplanXml(tenant, params) {
        const client = await (0, database_1.getClient)();
        try {
            // Recupera trunks attivi del tenant
            const trunksResult = await client.query('SELECT * FROM sip_trunks WHERE tenant_id = $1 AND status = $2 ORDER BY name LIMIT 1', [tenant.id, 'active']);
            const trunk = trunksResult.rows.length > 0 ? trunksResult.rows[0] : null;
            const gatewayName = trunk ? `trunk_${trunk.id}` : 'messagenet';
            // Recupera IVR menus
            const ivrResult = await client.query('SELECT * FROM ivr_menus WHERE tenant_id = $1 AND enabled = true', [tenant.id]);
            // Recupera call queues
            const queuesResult = await client.query('SELECT * FROM call_queues WHERE tenant_id = $1 AND enabled = true', [tenant.id]);
            // Recupera voicemail boxes
            const voicemailResult = await client.query('SELECT * FROM voicemail_boxes WHERE tenant_id = $1 AND enabled = true', [tenant.id]);
