...
exports.getESLService = getESLService;
exports.initializeESLService = initializeESLService;
 ...
 */
class ESLService extends events_1.EventEmitter {
    constructor() {
        super();
    ...
    async connect() {
        ...
        try {
            ...
            this.connection = new esl_1.FreeSwitchClient({
                host: this.host,
                port: this.port,
    ...
    async handleChannelCreate(event) {
        ...
        try {
            ...
            const tenantResult = await client.query('SELECT id FROM tenants WHERE slug = $1 LIMIT 1', [context]);
     ...
     */
    async handleChannelBridge(event) {
        const client = await (0, database_1.getClient)();
        try {
            const callUuid = event.getHeader('Unique-ID');
            await client.query(`
     ...
     */
    async handleChannelHangup(event) {
        const client = await (0, database_1.getClient)();
        try {
            const callUuid = event.getHeader('Unique-ID');
     ...
     */
    async sendBgapi(command) {
        if (!this.connection) {
            throw new Error('ESL not connected');
        }
        return new Promise((resolve, reject) => {
            // @ts-ignore - FreeSwitchClient types are incomplete
            this.connection.bgapi(command, (response) => {
     ...
     * Transfer a call
     */
    async transferCall(uuid, destination) {
...
// Singleton instance
let eslServiceInstance = null;
function getESLService() {
    if (!eslServiceInstance) {
        eslServiceInstance = new ESLService();
    }
    return eslServiceInstance;
}
async function initializeESLService() {
